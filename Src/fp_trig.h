...
/*
 * Computes square root of sine of x, measured in radians, using a lookup table
 *
 * Parameters:
 *    x     Expressed in radians * TRIG_SCALE_FACTOR
 *
 * Returns:
 *    sqrt(sin(x)) * 2 * TRIG_SCALE_FACTOR if x >=0 otherwise 0
 *
 * Therefore output will be in the range [0, 2*TRIG_SCALE_FACTOR]
 */
